...
def patch_epub_container(epub_path):
    container_xml_path = 'META-INF/container.xml'
    temp_epub_path = epub_path + '.patched'

    try:
        with zipfile.ZipFile(epub_path, 'r') as original_epub:
            file_list = original_epub.namelist()
            if container_xml_path in file_list:
                logging.info(f"'{container_xml_path}' already exists in {os.path.basename(epub_path)}. No patching needed.")
                return epub_path # No patch needed

            logging.info(f"'{container_xml_path}' missing in {os.path.basename(epub_path)}. Patching...")
...
'''
        ...
        logging.info(f"Patched EPUB and replaced original at {epub_path}")
        return epub_path
    except zipfile.BadZipFile:
        logging.error(f"The file at {epub_path} is not a valid zip archive.")
        raise
    except Exception as e:
        logging.error(f"An unexpected error occurred during patching: {e}")
        raise
...
class EpubToPdfConverter:
    def __init__(self):
        ...
        if font_path and os.path.exists(font_path):
            ...
            # Fallback to a common path, though it might fail if not present
...
EPUB to PDF Converter"""
    ...
    def convert_epub_to_pdf(self, conversion_id, params):
        ...
        try:
            ...
            epub_path = patch_epub_container(epub_path)
            ...
            # The main cleanup will handle the temp_dir
...
def start_conversion():
    ...
    thread_args = (conversion_id, params)
    target_func = converter.convert_epub_to_pdf
    if recipient_email:
        thread_args = (conversion_id, params, recipient_email)
        target_func = converter.convert_epub_to_pdf_and_email
...
def download_pdf(conversion_id):
    ...
    if not status or status.get('status') != 'completed':
        return jsonify({'error': 'Conversion not ready or failed'}), 400
...
# You would typically run this cleanup in a separate thread or using a scheduler
# For simplicity, a manual endpoint is provided here.
