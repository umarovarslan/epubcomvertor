...
class EmailSender:
    ...
    def send_pdf_email(self, recipient_email, pdf_path, book_title, page_count):
...
Please find your converted PDF attached to this email.
...
class PDFPageCounter:
    """Handle PDF page counting functionality"""
    ...
    def count_pages(pdf_path):
        ...
        try:
            if PYPDF2_AVAILABLE:
                ...
                # Fallback method using file size estimation
                # This is less accurate but works without additional dependencies
                file_size = os.path.getsize(pdf_path)
                # Rough estimation: 1 page ≈ 50KB (very approximate)
                estimated_pages = max(1, file_size // 51200)  # 50KB in bytes
...
class EpubToPdfConverter:
    ...
    def convert_epub_to_pdf(self, conversion_id, params):
        """Main conversion logic with enhanced features"""
        try:
            ...
            # Handle new margin parameters
            top_bottom_margin = float(params.get('top_bottom_margin', 1.0))
            inner_margin = float(params.get('inner_margin', 1.25))
            outer_margin = float(params.get('outer_margin', 0.75))
            ...
            # Use new margin parameters
            doc.leftMargin = inner_margin * inch
            doc.rightMargin = outer_margin * inch
            doc.topMargin = top_bottom_margin * inch
            doc.bottomMargin = top_bottom_margin * inch
            ...
            page_count = page_counter.count_pages(pdf_filename)
            conversion_status[conversion_id]['page_count'] = page_count
            
            # Send email if requested
            if send_email and recipient_email:
                conversion_status[conversion_id]['progress'] = 95
                conversion_status[conversion_id]['message'] = 'Sending email...'
                
                email_sender = EmailSender()
                email_success, email_message = email_sender.send_pdf_email(
                    recipient_email, pdf_filename, book_title, page_count
                )
                
                conversion_status[conversion_id]['email_status'] = {
            ...
            conversion_status[conversion_id] = {
                ...
                'email_status': conversion_status[conversion_id].get('email_status'),
                'created_at': datetime.now()
            }
...
def start_conversion():
    """Start EPUB to PDF conversion with enhanced features"""
    try:
...
def get_conversion_status(conversion_id):
    """Get conversion status with page count"""
