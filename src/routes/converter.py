...
EPUB to PDF Converter"""
    ...
    def _get_paragraph_style(self, tag_name, styles, font_size, line_spacing):
        """Helper to get appropriate paragraph style based on tag name."""
        leading = font_size * line_spacing
        if tag_name == \'h1\':
            return ParagraphStyle(\'H1\', parent=styles[\'h1\'], fontName=\'DejaVu-Sans\',
                                    fontSize=20, leading=24, spaceAfter=12, alignment=TA_CENTER)
        elif tag_name == \'h2\':
            return ParagraphStyle(\'H2\', parent=styles[\'h2\'], fontName=\'DejaVu-Sans\',
                                    fontSize=18, leading=22, spaceAfter=10)
        elif tag_name == \'h3\':
            return ParagraphStyle(\'H3\', parent=styles[\'h3\'], fontName=\'DejaVu-Sans\',
                                    fontSize=16, leading=20, spaceAfter=8)
        elif tag_name == \'blockquote\':
            return ParagraphStyle(\'Blockquote\', parent=styles[\'Normal\'], fontName=\'DejaVu-Sans\',
                                    fontSize=font_size, leading=leading, leftIndent=20, rightIndent=20,
                                    spaceBefore=6, spaceAfter=6, textColor=colors.darkgrey)
        else:
            return ParagraphStyle(\'BodyText\', parent=styles[\'Normal\'], fontName=\'DejaVu-Sans\',
                                      fontSize=font_size, leading=leading, alignment=TA_JUSTIFY)

    def process_html_node(self, node, styles, body_style, h1_style, h2_style, h3_style, image_map, frame_width, frame_height, font_size, line_spacing):
        """Recursively process a single HTML node (Tag or NavigableString) and return flowables."""
        flowables = []

        if isinstance(node, NavigableString):
            text = str(node).strip()
            if text:
                # Use body_style for raw text nodes
            ...
            if tag_name in [\'h1\', \'h2\', \'h3\', \'h4\', \'h5\', \'h6\']:
                ...
                if text:
                    ...
                    if level == 1: flowables.append(Paragraph(text, h1_style))
                    elif level == 2: flowables.append(Paragraph(text, h2_style))
                    else: flowables.append(Paragraph(text, h3_style))
                    flowables.append(Spacer(1, 0.15 * inch))

            elif tag_name == \'p\':
                # Process paragraph content, including inline formatting
                paragraph_content = self._process_inline_html(node.decode_contents(), body_style)
                if paragraph_content.strip():
                    flowables.append(Paragraph(paragraph_content, body_style))
                    flowables.append(Spacer(1, 0.1 * inch))

            elif tag_name == \'img\' and node.get(\'src\'):
                img_src_base = os.path.basename(node[\'src\'])
                if img_src_base in image_map:
                    try:
                        ...
                        logging.warning(f"Error processing image {node[\'src\]}: {e}")

            elif tag_name in [\'ul\', \'ol\']:
                list_items = []
                for li in node.find_all(\'li\', recursive=False):
                    # Recursively process list item content
                    li_flowables = self.process_html_node(li, styles, body_style, h1_style, h2_style, h3_style, image_map, frame_width, frame_height, font_size, line_spacing)
                    # Convert flowables back to a single string for ListItem, or handle as complex flowables
                    # For simplicity, let's join text content for now. A more robust solution might need a custom ListFlowable item.
                    li_text = \'\'.join([p.text for p in li_flowables if isinstance(p, Paragraph)])
                    if li_text:
                        list_items.append(ListItem(Paragraph(li_text, body_style), leftIndent=20))
                
                if list_items:
                    bullet_style = \'bullet\' if tag_name == \'ul\' else \'1\'
                    flowables.append(ListFlowable(list_items, bulletType=bullet_style))
                    flowables.append(Spacer(1, 0.2 * inch))

            elif tag_name == \'blockquote\':
                quote_content = self._process_inline_html(node.decode_contents(), body_style)
                if quote_content.strip():
                    flowables.append(Paragraph(f\'<i>"{quote_content}"</i>\', self._get_paragraph_style(\'blockquote\', styles, font_size, line_spacing)))
                    flowables.append(Spacer(1, 0.1 * inch))
                ...
                # For other tags (like div, span, etc.), recursively process their children
                for child in node.children:
                    flowables.extend(self.process_html_node(child, styles, body_style, h1_style, h2_style, h3_style, image_map, frame_width, frame_height, font_size, line_spacing))
        
        return flowables

    def _process_inline_html(self, html_string, base_style):
        """Process inline HTML (like strong, em) within a block and return a ReportLab-compatible string."""
        # This is a simplified approach. For full HTML support, a more complex HTML parser for ReportLab is needed.
        # For now, we'll convert <b>, <i> tags.
        html_string = html_string.replace(\'<strong>\', \'<b>\').replace(\'</strong>\', \'</b>\')
        html_string = html_string.replace(\'<em>\', \'<i>\').replace(\'</em>\', \'</i>\')
        html_string = html_string.replace(\'<b>\', \'<b>\').replace(\'</b>\', \'</b>\') # Ensure b tags are handled
        html_string = html_string.replace(\'<i>\', \'<i>\').replace(\'</i>\', \'</i>\') # Ensure i tags are handled
        
        # Remove any other tags that are not explicitly handled by ReportLab's Paragraph
        clean = re.compile(\'<[^>]+>\')
        return re.sub(clean, \'\', html_string) # This will remove all other tags, potentially losing some formatting
    ...
    def build_story(self, doc, book_title, author_name, book_description, toc_items, content_map,
        ...
        body_style = self._get_paragraph_style(\'p\', styles, font_size, line_spacing)
        h1_style = self._get_paragraph_style(\'h1\', styles, font_size, line_spacing)
        h2_style = self._get_paragraph_style(\'h2\', styles, font_size, line_spacing)
        h3_style = self._get_paragraph_style(\'h3\', styles, font_size, line_spacing)
        ...
        for i, item in enumerate(toc_items):
            ...
            title_with_anchor = f\'<a><a name="{bookmark_key}"/>{item.title}</a>\'
            ...
            if chapter_html:
                ...
                for child_node in soup.children:
                    chapter_flowables = self.process_html_node(child_node, styles, body_style, h1_style, h2_style, h3_style, image_map, frame_width, frame_height, font_size, line_spacing)
                    chapter_content_story.extend(chapter_flowables)

        for title, key in toc_links:
            toc_page_content.append(Paragraph(f\'<a><link href="#{key}">{title}</link></a>\', toc_style))
    ...
    def convert_epub_to_pdf(self, conversion_id, params):
        ...
        try:
            ...
            # Calculate price: ((page number)*0.04)+22
            price = (page_count * 0.04) + 22
            ...
            conversion_status[conversion_id] = {
                \'status\': \'completed\',
                \'progress\': 100,
                \'message\': f\'PDF generation complete! ({page_count} pages, {price:.2f}rmb)\,',
    ...
    def convert_epub_to_pdf_and_email(self, conversion_id, params, recipient_email):
        ...
        try:
            ...
            if conversion_status[conversion_id][\'status\'] == \'completed\':
                ...
                price = conversion_status[conversion_id].get(\'price\', 0)
                ...
                if self.send_email_with_pdf(recipient_email, pdf_path, book_title, page_count, custom_email_body):
                    conversion_status[conversion_id][\'message\'] = f\'PDF sent successfully to {recipient_email}! ({page_count} pages, {price:.2f}rmb)\,',
