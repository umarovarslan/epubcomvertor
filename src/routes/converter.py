...
EPUB to PDF Converter"""
        ...
        for element in soup.contents:
            if isinstance(element, Tag):
                if element.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                    ...
                    # Recursively process div content but stay careful with nesting
                    div_flowables = self.process_html_content(element.decode_contents(), body_style, h1_style, h2_style, h3_style, image_map, frame_width, frame_height)
                    ...
                    # Fallback for other elements: check if it has text directly
                    text = element.get_text(strip=True)
                    if text and not element.find(): # Only if no child tags
                        flowables.append(Paragraph(text, body_style))
    ...
    def build_story(self, doc, book_title, author_name, book_description, toc_items, content_map,
        ...
        for i, item in enumerate(toc_items):
            ...
            if chapter_html:
                # Use the improved process_html_content to avoid duplications
                chapter_flowables = self.process_html_content(chapter_html, body_style, h1_style, h2_style, h3_style, image_map, frame_width, frame_height)
